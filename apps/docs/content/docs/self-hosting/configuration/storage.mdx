...
Database storage is the default option and requires no additional configuration. Documents are stored as base64-encoded data directly in the embedded SQLite database (Hanzo Base), in the same `sign.db` file as the rest of your data.
...
- Simple deployment
- Backed up together with the rest of the database (single file snapshot)

</Tab>
<Tab value="Limitations">

- Grows the `sign.db` file significantly
- Slower for large files
- File snapshots take longer as the database grows
...
```bash
aws s3 mb s3://your-hanzo-sign-bucket --region us-east-1
```
...
[
  {
    ...
    "AllowedOrigins": ["https://your-hanzo-sign-domain.com"],
    "ExposeHeaders": ["ETag"]
  }
]
```
...
```bash
aws s3api put-bucket-cors --bucket your-hanzo-sign-bucket --cors-configuration file://cors.json
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::your-hanzo-sign-bucket/*"
    }
  ]
}
```
...
[Hanzo S3](https://github.com/hanzoai/s3) is our self-hosted S3-compatible
object storage server (SeaweedFS-derived, Apache-2.0).
...
The `mini` command is the all-in-one server. `S3_BUCKET` creates the bucket on
startup, so there is no separate bucket-creation step.
...
docker run -d \
  --name s3 \
  -p 9000:9000 \
  -e AWS_ACCESS_KEY_ID=hanzo-sign \
  -e AWS_SECRET_ACCESS_KEY=change-me \
  -e S3_BUCKET=hanzo-sign \
  -v s3_data:/data \
  ghcr.io/hanzoai/s3:v1.0.14 mini -dir=/data -s3.port=9000
```
...
Hanzo S3 reads `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` and registers them
as an admin identity. `S3_BUCKET` accepts a comma-separated list.
...
NEXT_PRIVATE_UPLOAD_ENDPOINT=http://s3:9000
NEXT_PRIVATE_UPLOAD_FORCE_PATH_STYLE=true
NEXT_PRIVATE_UPLOAD_ACCESS_KEY_ID=hanzo-sign
NEXT_PRIVATE_UPLOAD_SECRET_ACCESS_KEY=change-me
NEXT_PRIVATE_UPLOAD_REGION=us-east-1
```
...
<Callout type="info">
  Set `NEXT_PRIVATE_UPLOAD_FORCE_PATH_STYLE=true` for Hanzo S3 and other
  S3-compatible services that don't support virtual-hosted bucket URLs.
</Callout>
...
When using database storage, plan for significant growth of the `sign.db` file
...
| ------------- | -------------------------------------- | ----------------------- |
| Storage cost  | Persistent volume holding `sign.db`    | S3 pricing (~$0.023/GB) |
| Transfer cost | Local disk I/O                         | S3 requests + egress    |
| Backup cost   | Larger `sign.db` snapshots             | Separate S3 backups     |
| Performance   | Degrades as the file grows             | Consistent              |

With database storage there is no managed-database bill: the documents live in
the same `sign.db` file on the persistent volume you already pay for. The
tradeoff is that the volume must grow with your document corpus, and snapshots
take longer.
...
- [Database Configuration](/docs/self-hosting/configuration/database) - Configure the SQLite database
...
- [Docker Compose](/docs/self-hosting/deployment/docker-compose) - Deploy with Hanzo S3 for local storage
