...
Hanzo eSign stores all of its data in an **embedded SQLite database (Hanzo Base)**
...
Hanzo eSign can store documents in the embedded SQLite database (Hanzo Base) or in S3-compatible storage.
...
Hanzo eSign uses a SQLite-backed job queue by default. Jobs (email delivery, document processing, webhook dispatch) are stored in the `BackgroundJob` table inside the embedded Base database and processed via internal HTTP requests. No external queue service like Redis is required.
