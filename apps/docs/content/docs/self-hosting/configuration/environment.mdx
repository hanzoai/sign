...
| `DATABASE_URL`                          | SQLite database file URL (e.g., `file:/opt/hanzo-sign/base/data/_dev/sign.db`)              |
| `BASE_PATH`                             | Directory root for embedded Base data on the persistent disk                               |
...
Hanzo Sign stores all of its data in an **embedded SQLite database (Hanzo Base)**
on the application's persistent disk. There is no external database server to
provision, connect, or tune. esign uses a single shared SQLite file with tenancy
enforced at the query layer (see HIP-0305), not a database-per-tenant split.

| Variable       | Required | Description                                                                |
| -------------- | -------- | -------------------------------------------------------------------------- |
| `BASE_PATH`    | Yes      | Directory root for Base data on the persistent disk                        |
| `DATABASE_URL` | Yes      | SQLite file URL. Defaults to a file under `BASE_PATH`                       |
...
Callback URL: `https://<your-domain>/v1/auth/callback/google`
...
Callback URL: `https://<your-domain>/v1/auth/callback/microsoft`
...
Hanzo Sign can store documents in the embedded SQLite database (Hanzo Base) or in S3-compatible storage.
...
Hanzo Sign uses a SQLite-backed job queue by default. Jobs (email delivery, document processing, webhook dispatch) are stored in the `BackgroundJob` table inside the embedded Base database and processed via internal HTTP requests. No external queue service like Redis is required.

| Variable                     | Description                                                                | Default |
| ---------------------------- | -------------------------------------------------------------------------- | ------- |
| `NEXT_PRIVATE_JOBS_PROVIDER` | Jobs provider: `local` (SQLite-backed queue) or `inngest` (managed service) | `local` |
...
| Variable                      | Description                                  | Default |
| ----------------------------- | -------------------------------------------- | ------- |
| `SIGN_DISABLE_TELEMETRY` | Set to `true` to disable anonymous telemetry | `false` |
...
| Variable                             | Description                                      |
| ------------------------------------ | ------------------------------------------------ |
| `NEXT_PRIVATE_SIGN_LICENSE_KEY` | License key for enterprise features              |
...
# Database (embedded Base SQLite on a persistent disk)
...
- [Database Configuration](/docs/self-hosting/configuration/database) - Set up the embedded SQLite database
