...
```bash
docker run -e SIGN_DISABLE_TELEMETRY=true ...
```

### After Disabling

Once you set `SIGN_DISABLE_TELEMETRY=true` and restart your server, no telemetry data will be sent. The telemetry client will not initialize, and no network requests will be made to our telemetry servers.
