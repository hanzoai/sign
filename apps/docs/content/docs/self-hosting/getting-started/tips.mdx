...
The default certificate path inside the container is `/opt/hanzo-sign/cert.p12`. You can change this with `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`.
...
| Endpoint                  | Purpose                              |
| ------------------------- | ------------------------------------ |
| `/v1/health`             | Returns 200 if the application is up |
| `/v1/certificate-status` | Detailed signing certificate status  |

Use `/v1/health` for container orchestration health checks (Docker, Kubernetes). Use `/v1/certificate-status` to debug signing issues. It reports whether a certificate is configured, its type, and any errors.
...
docker pull hanzo-sign/hanzo-sign:1.8.0
...
- **Upgrades are simple**: pull the new image, restart the container, and migrations apply automatically against the SQLite file.
- **Always snapshot the `sign.db` file before upgrading**: if a migration fails, the file copy is your rollback. See [Backups](/docs/self-hosting/maintenance/backups).
...
Documents are stored in the embedded SQLite database (Hanzo Base) by default (`NEXT_PUBLIC_UPLOAD_TRANSPORT=database`), as base64 in the same `sign.db` file. This works well for small-to-medium deployments and keeps your infrastructure simple.

For high-volume deployments, switch to S3-compatible storage to keep the `sign.db` file lean:

```bash
NEXT_PUBLIC_UPLOAD_TRANSPORT=s3
NEXT_PRIVATE_UPLOAD_BUCKET=hanzo-sign-documents
...
Hanzo Sign uses a SQLite-backed job queue by default. No Redis, no external message broker. The job system uses the embedded Base database to store and process background tasks like email delivery and document processing.
