...
Any S3-compatible provider works: Hanzo S3, AWS S3, Cloudflare R2, DigitalOcean Spaces.
...
Hanzo eSign uses a SQLite-backed job queue by default. No Redis, no external message broker. The job system uses the embedded Base database to store and process background tasks like email delivery and document processing.
