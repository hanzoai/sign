...
description: Deploy Hanzo Sign with Docker Compose. A single application container with an embedded SQLite database on a named volume — production-ready for most self-hosted deployments.
---
...
<Accordions type="multiple">
  ...
  <Accordion title="Certificate errors">
    Check certificate status: `curl http://localhost:3000/v1/certificate-status`. Common issues:
  ...
  <Accordion title="Application unreachable">
    ...
    test locally: `curl http://localhost:3000/v1/health`; check firewall rules allow traffic on the
    configured port.
  </Accordion>
