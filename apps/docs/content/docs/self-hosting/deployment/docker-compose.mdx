...
```bash
# Database (used by both database and hanzo-sign services)
...
NEXT_PRIVATE_DATABASE_URL=postgresql://hanzo-sign:your-secure-database-password@database:5432/hanzo-sign
...
```
NAME                             STATUS              PORTS
hanzo-sign-production-database-1  running (healthy)   5432/tcp
...
```bash
# Manual backup
docker compose exec database pg_dump -U hanzo-sign hanzo-sign > backup.sql

# Restore from backup
docker compose exec -T database psql -U hanzo-sign hanzo-sign < backup.sql
```
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@your-db-host:5432/hanzo-sign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@your-db-host:5432/hanzo-sign
```
...
<Accordions type="multiple">
  ...
  <Accordion title="Database connection errors">
    ...
    database psql -U hanzo-sign -d hanzo-sign -c "SELECT 1"`.
  </Accordion>
