---
title: Docker
description: Run Hanzo eSign as a standalone Docker container with an embedded SQLite database on a persistent volume. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
Hanzo eSign stores all data in an embedded SQLite database (Hanzo Base) on the
...
The Hanzo eSign container's code is stateless, but its data is not — all
