...
- Node.js 20 or later
- npm
- A persistent disk for the embedded SQLite database (no separate database server — set `BASE_PATH` to a directory on durable storage)
...
Navigate to the `hanzo-sign` folder and create a `.env` file:
...
Type=simple
User=www-data
WorkingDirectory=/var/www/hanzo-sign
EnvironmentFile=/var/www/hanzo-sign/.env
...
```bash
sudo systemctl enable hanzo-sign
sudo systemctl start hanzo-sign
```
