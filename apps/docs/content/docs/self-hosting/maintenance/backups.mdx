...
```bash
pg_dump -h localhost -U hanzo-sign -d hanzo-sign -F c -f hanzo-sign_backup.dump
```
...
```bash
docker compose exec database pg_dump -U hanzo-sign -F c hanzo-sign > hanzo-sign_backup.dump
```
...
```bash
pg_dump -h localhost -U hanzo-sign -d hanzo-sign -F c \
  -f "hanzo-sign_$(date +%Y%m%d_%H%M%S).dump"
```
...
```bash
pg_dump -h localhost -U hanzo-sign -d hanzo-sign -F p -f hanzo-sign_backup.sql
```
...
```bash
pg_dump -h localhost -U hanzo-sign -d hanzo-sign | gzip > hanzo-sign_backup.sql.gz
```
