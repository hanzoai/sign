...
| --------------------- | ----------------------------- | --------------------------------------------------- |
| SQLite database       | `sign.db` file under `BASE_PATH` | Users, documents, signatures, audit logs, settings |
| Signing certificate   | File system or secret manager | Private key for signing documents                   |
| Document storage (S3) | S3 bucket or the SQLite database | Uploaded PDFs and signed documents               |
...
The embedded Base database (`sign.db`) contains:
...
- The `BackgroundJob` queue

If using database storage (`NEXT_PUBLIC_UPLOAD_TRANSPORT=database`), the same file also contains the actual PDF files.

---

## Database Backups

The database is a single SQLite file under `BASE_PATH`, so backups are file
snapshots — no `pg_dump`, no replication topology. Use the `sqlite3 .backup`
command, which produces a consistent copy even while the app is running.
...
```bash
sqlite3 /opt/hanzo-sign/base/data/_dev/sign.db \
  ".backup '/backup/hanzo-sign/sign-$(date +%Y%m%d_%H%M%S).db'"
```
...
docker compose exec hanzo-sign \
  sqlite3 /opt/hanzo-sign/base/data/_dev/sign.db \
  ".backup '/opt/hanzo-sign/base/data/sign-backup.db'"
docker compose cp \
  hanzo-sign:/opt/hanzo-sign/base/data/sign-backup.db \
  "./sign-$(date +%Y%m%d_%H%M%S).db"
```

</Tab>
</Tabs>

The `.backup` command copies the live database (including the WAL) into a new,
self-contained file. It is safe to run while Hanzo eSign is serving traffic.

### Plain File Copy (App Stopped)

If the app is stopped, an ordinary `cp` is sufficient and fastest:

```bash
cp /opt/hanzo-sign/base/data/_dev/sign.db \
   "/backup/hanzo-sign/sign-$(date +%Y%m%d_%H%M%S).db"
```

<Callout type="warn">
  A plain `cp` of a live database can capture a torn write. Use `sqlite3 .backup`
  for online copies; reserve `cp` for when the app is stopped.
</Callout>

### Compressed Backup

Compress snapshots to save storage space:

```bash
sqlite3 /opt/hanzo-sign/base/data/_dev/sign.db ".backup '/tmp/sign.db'"
gzip -c /tmp/sign.db > "hanzo-sign_$(date +%Y%m%d_%H%M%S).db.gz" && rm /tmp/sign.db
```
...
```bash
cp /opt/hanzo-sign/cert.p12 /backup/hanzo-sign/cert.p12
```
...
```bash
echo "$NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS" | base64 -d > /backup/hanzo-sign/cert.p12
```
...
<Tabs items={['AWS S3', 'S3-Compatible (Hanzo S3, etc.)']}>
<Tab value="AWS S3">
...
```bash
aws s3 sync s3://your-hanzo-sign-bucket s3://your-backup-bucket
# or
aws s3 sync s3://your-hanzo-sign-bucket /backup/hanzo-sign/documents/
```

</Tab>
<Tab value="S3-Compatible (Hanzo S3, etc.)">

Use the same commands with the `--endpoint-url` flag:

```bash
aws s3 sync s3://your-bucket /backup/hanzo-sign/documents/ \
...
```bash
#!/bin/bash
# hanzo-sign-backup.sh

set -e

# Configuration
BACKUP_DIR="/backup/hanzo-sign"
RETENTION_DAYS=30
...
S3_BUCKET="your-hanzo-sign-bucket"
...
if [ -f "/opt/hanzo-sign/cert.p12" ]; then
  echo "Backing up certificate..."
  cp /opt/hanzo-sign/cert.p12 "$BACKUP_DIR/$TIMESTAMP/cert.p12"
fi
...
tar -czf "$BACKUP_DIR/hanzo-sign_$TIMESTAMP.tar.gz" -C "$BACKUP_DIR" "$TIMESTAMP"
rm -rf "$BACKUP_DIR/$TIMESTAMP"

# Calculate checksum
sha256sum "$BACKUP_DIR/hanzo-sign_$TIMESTAMP.tar.gz" > "$BACKUP_DIR/hanzo-sign_$TIMESTAMP.sha256"

# Remove old backups
echo "Removing backups older than $RETENTION_DAYS days..."
find "$BACKUP_DIR" -name "hanzo-sign_*.tar.gz" -mtime +$RETENTION_DAYS -delete
find "$BACKUP_DIR" -name "hanzo-sign_*.sha256" -mtime +$RETENTION_DAYS -delete

echo "Backup completed: hanzo-sign_$TIMESTAMP.tar.gz"
```
...
```bash
#!/bin/bash
# hanzo-sign-backup-docker.sh

set -e

BACKUP_DIR="/backup/hanzo-sign"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
COMPOSE_PROJECT="hanzo-sign-production"
...
docker compose -p "$COMPOSE_PROJECT" exec -T database \
  pg_dump -U hanzo-sign -F c hanzo-sign > "$BACKUP_DIR/database_$TIMESTAMP.dump"

# Certificate backup (copy from container if needed)
docker compose -p "$COMPOSE_PROJECT" cp \
  hanzo-sign:/opt/hanzo-sign/cert.p12 "$BACKUP_DIR/cert_$TIMESTAMP.p12" 2>/dev/null || true
...
```bash
chmod +x /usr/local/bin/hanzo-sign-backup.sh
```
...
0 2 * * * /usr/local/bin/hanzo-sign-backup.sh >> /var/log/hanzo-sign-backup.log 2>&1

# Hourly backup (for high-activity instances)
0 * * * * /usr/local/bin/hanzo-sign-backup-hourly.sh >> /var/log/hanzo-sign-backup.log 2>&1
```

</Tab>
<Tab value="Systemd timer">

Create `/etc/systemd/system/hanzo-sign-backup.service`:
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/hanzo-sign-backup.sh
User=root
```

Create `/etc/systemd/system/hanzo-sign-backup.timer`:
...
systemctl enable --now hanzo-sign-backup.timer
```
...
```bash
pg_restore -h localhost -U hanzo-sign -d hanzo-sign -c hanzo-sign_backup.dump
```

Docker Compose:

```bash
docker compose exec -T database pg_restore -U hanzo-sign -d hanzo-sign -c < hanzo-sign_backup.dump
```
...
```bash
psql -h localhost -U hanzo-sign -d hanzo-sign < hanzo-sign_backup.sql
```

For compressed backups:

```bash
gunzip -c hanzo-sign_backup.sql.gz | psql -h localhost -U hanzo-sign -d hanzo-sign
```
...
```bash
cp /backup/hanzo-sign/cert.p12 /opt/hanzo-sign/cert.p12
chmod 644 /opt/hanzo-sign/cert.p12
chown 1001:1001 /opt/hanzo-sign/cert.p12
```
...
```bash
aws s3 sync /backup/hanzo-sign/documents/ s3://your-hanzo-sign-bucket
```
...
```bash
docker compose exec -T database pg_restore -U hanzo-sign -d hanzo-sign -c < backup.dump
```
...
```bash
docker compose cp cert.p12 hanzo-sign:/opt/hanzo-sign/cert.p12
```
...
BACKUP_FILE="$1"
TEST_DB="hanzo-sign_restore_test"
...
```bash
# Check checksum
sha256sum -c hanzo-sign_backup.sha256

# Verify pg_dump format
pg_restore --list hanzo-sign_backup.dump > /dev/null && echo "Backup is valid"
```
...
```bash
aws s3 cp hanzo-sign_backup.tar.gz s3://your-backup-bucket/hanzo-sign/
```
...
```bash
# Encrypt
gpg --symmetric --cipher-algo AES256 hanzo-sign_backup.tar.gz

# Decrypt
gpg --decrypt hanzo-sign_backup.tar.gz.gpg > hanzo-sign_backup.tar.gz
```
