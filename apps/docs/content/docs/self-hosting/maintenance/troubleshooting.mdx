...
```bash
kubectl logs -l app.kubernetes.io/name=hanzo-sign -n hanzo-sign -f
```
...
```bash
# Docker - find errors
docker logs hanzo-sign 2>&1 | grep -i error

# Docker - find warnings
docker logs hanzo-sign 2>&1 | grep -i warn

# Show last 100 lines
docker logs --tail 100 hanzo-sign
```
...
```bash
docker exec hanzo-sign printenv | grep NEXT
```
...
```bash
docker compose exec hanzo-sign printenv | grep NEXT
```
...
```bash
docker inspect hanzo-sign --format='{{.State.ExitCode}}'
```
...
```bash
# Docker
docker stats hanzo-sign

# Kubernetes
kubectl top pod -n hanzo-sign
```
...
```
Error: password authentication failed for user "hanzo-sign"
```
...
```sql
ALTER USER hanzo-sign WITH PASSWORD 'newpassword';
```
...
```
Error: database "hanzo-sign" does not exist
```
...
```
Error: too many connections for role "hanzo-sign"
```
...
```
postgresql://user:password@host:5432/hanzo-sign?connection_limit=5
```
...
```bash
docker exec hanzo-sign printenv | grep SMTP
```
...
```bash
docker exec hanzo-sign nc -zv smtp.example.com 587
```
...
```bash
docker exec hanzo-sign printenv | grep SIGNING
```
...
```bash
docker exec hanzo-sign ls -la /opt/hanzo-sign/cert.p12
```
...
```
Error: EACCES: permission denied, open '/opt/hanzo-sign/cert.p12'
```
...
```bash
docker exec hanzo-sign cat /etc/resolv.conf
```
...
- **[Discord](https://esign.hanzo.ai/discord)**: Join the Hanzo Sign community Discord
...
<Accordions type="multiple">
  <Accordion title="Check application logs for errors">
    Run `docker logs hanzo-sign` or `docker compose logs hanzo-sign` (or `kubectl logs` for
  ...
  <Accordion title="Test health endpoint responds">
    Run `curl http://localhost:3000/v1/health` and verify it returns a successful response.
  </Accordion>
  ...
  <Accordion title="Verify certificate is valid and accessible">
    Check `curl http://localhost:3000/v1/certificate-status` and ensure file permissions are
    correct.
  </Accordion>
