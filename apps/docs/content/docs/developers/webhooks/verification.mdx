...
When you configure a webhook with a secret, Hanzo Sign includes that secret in every webhook request via the `X-Hanzo-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-Hanzo-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-Hanzo-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-Hanzo-Secret` header from the incoming request
  </Step>
  <Step>
...
const WEBHOOK_SECRET = process.env.SIGN_WEBHOOK_SECRET;
...
app.post('/webhooks/hanzo-sign', (req, res) => {
  const receivedSecret = req.headers['x-hanzo-sign-secret'];
...
WEBHOOK_SECRET = os.environ.get('SIGN_WEBHOOK_SECRET')
...
def handle_webhook():
    received_secret = request.headers.get('X-Hanzo-Secret', '')
...
app.post('/webhooks/hanzo-sign', webhookLimiter, (req, res) => {
  // Handle webhook...
});
```
