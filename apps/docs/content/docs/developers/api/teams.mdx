...
```typescript
const TEAM_API_TOKEN = process.env.SIGN_TEAM_API_TOKEN;
...
# Filter by status
curl -X GET "https://app.esign.hanzo.ai/v1/rpc/envelope?status=PENDING" \
  -H "Authorization: api_team_xxxxxxxxxxxxxxxx"
````
...
<Tab value="TypeScript">
```typescript
const response = await fetch('https://app.esign.hanzo.ai/v1/rpc/envelope', {
  method: 'GET',
  headers: {
    Authorization: TEAM_API_TOKEN,
  },
...
<Tab value="curl">
```bash
curl -X POST "https://app.esign.hanzo.ai/v1/rpc/template/create" \
  -H "Authorization: api_team_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: multipart/form-data" \
  -F 'payload={
    "title": "NDA Template",
...
const response = await fetch('https://app.esign.hanzo.ai/v1/rpc/template/create', {
  method: 'POST',
  headers: {
    Authorization: TEAM_API_TOKEN,
  },
...
<Tab value="curl">
```bash
curl -X GET "https://app.esign.hanzo.ai/v1/rpc/template" \
  -H "Authorization: api_team_xxxxxxxxxxxxxxxx"
````
...
<Tab value="TypeScript">
```typescript
const response = await fetch('https://app.esign.hanzo.ai/v1/rpc/template', {
  method: 'GET',
  headers: {
    Authorization: TEAM_API_TOKEN,
  },
...
const salesResponse = await fetch('https://app.esign.hanzo.ai/v1/rpc/envelope?status=PENDING', {
  headers: { Authorization: SALES_TEAM_TOKEN },
});
const salesDocs = await salesResponse.json();

// Get completed documents from legal team
const legalResponse = await fetch('https://app.esign.hanzo.ai/v1/rpc/envelope?status=COMPLETED', {
