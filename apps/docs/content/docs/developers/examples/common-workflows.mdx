...
app.post('/webhooks/hanzo-sign', (req, res) => {
  ...
  const secret = req.headers['x-hanzo-sign-secret'] as string;
...
<Steps>
  ...
  <Step>
    Enter your webhook URL (e.g. <code>https://your-app.com/webhooks/hanzo-sign</code>)
  </Step>
  <Step>
  ...
  <Step>
    Enter your webhook secret (the same value stored in <code>SIGN_WEBHOOK_SECRET</code>)
  </Step>
  <Step>
    Click **Create Webhook**
  </Step>
...
class Hanzo SignApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof Hanzo SignApiError) {
