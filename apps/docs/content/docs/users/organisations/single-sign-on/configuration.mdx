...
Hanzo Sign supports any identity provider that implements the OpenID Connect (OIDC) protocol. Common providers include:
...
- **Redirect URI**: The value from the Hanzo Sign SSO settings page
...
Back in Hanzo Sign, enter the following:
...
Hanzo Sign verifies their identity with your provider.
...
- Cannot set a Hanzo Sign password (they must use SSO)
...
<Callout type="info">
  SSO-only enforcement is not currently available. Users who have an existing Hanzo Sign account with a password can still sign in with their original method in addition to SSO.
</Callout>
...
<Accordions type="multiple">
  ...
  - Copy the redirect URI exactly from Hanzo Sign's SSO settings
  ...
  <Accordion title="User already exists">
    If a user already has a Hanzo Sign account with the same email, their existing account will be linked to your team and they can sign in through your SSO portal going forward.
  </Accordion>
</Accordions>
