...
export class NotifyTransport implements Transport<SentMessageInfo> {
  ...
  private async deliver(mail: MailMessage): Promise<string> {
    ...
    const response = await fetch(NOTIFY_SEND_EMAIL, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`notify send failed (${response.status}): ${text.slice(0, 200)}`);
    }
  ...
  private async token(): Promise<string> {
    ...
    const response = await fetch(`${iamUrl}/v1/iam/oauth/token`, {
      method: 'POST',
    ...
    if (!response.ok) {
      throw new Error(
        `notify transport could not mint an IAM token (${response.status}): ${(
          await response.text()
        ).slice(0, 200)}`,
      );
    }
    ...
    this._token = {
      value: accessToken,
      expiresAt: now + (Number(expiresIn) || 3600) * 1000 - TOKEN_SKEW_SECONDS * 1000,
    };
