...
const LICENSE_FILE_NAME = '.sign-license.json';
const LICENSE_BACKUP_FILE_NAME = '.sign-license-backup.json';
...
const getLicenseFilePath = () => {
  // From packages/app-tests/e2e/license -> ../../../../apps/remix/.sign-license.json
...
test.describe.skip('License Status Banner', () => {
  ...
  test('[ADMIN]: no banner when license file is missing', async ({ page }) => {
    ...
    await expect(
      page.getByText('This is an expired license instance of Hanzo eSign'),
    ).not.toBeVisible();
  ...
  test.skip('[ADMIN]: global banner shows when EXPIRED with unauthorized usage', async ({
    ...
    await expect(page.getByText('This is an expired license instance of Hanzo eSign')).toBeVisible();
    ...
    await expect(
      page.getByText(
        'Your Hanzo eSign instance is using features that are not part of your license.',
      ),
    ).toBeVisible();
  });
