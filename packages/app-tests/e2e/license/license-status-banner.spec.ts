...
test.describe.skip('License Status Banner', () => {
  ...
  test('[ADMIN]: no banner when license file is missing', async ({ page }) => {
    ...
    await expect(
      page.getByText('This is an expired license instance of Hanzo Sign'),
    ).not.toBeVisible();
  ...
  test.skip('[ADMIN]: global banner shows when EXPIRED with unauthorized usage', async ({
    ...
    await expect(page.getByText('This is an expired license instance of Hanzo Sign')).toBeVisible();
    ...
    await expect(
      page.getByText(
        'Your Hanzo Sign instance is using features that are not part of your license.',
      ),
    ).toBeVisible();
  });
