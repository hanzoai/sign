...
test('[ADMIN]: promote member to owner', async ({ page }) => {
  ...
  const adminMemberEmail = `admin-member-${nanoid()}@test.esign.hanzo.ai`;
...
test('[ADMIN]: multiple promotions in sequence', async ({ page }) => {
  ...
  const member1Email = `member1-${nanoid()}@test.esign.hanzo.ai`;
  const member2Email = `member2-${nanoid()}@test.esign.hanzo.ai`;
