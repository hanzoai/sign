...
import { AccessAuth2FAEmailTemplate } from '@hanzo/esign-email/templates/access-auth-2fa';
...
export const send2FATokenEmail = async ({ token, envelopeId }: Send2FATokenEmailOptions) => {
  ...
  // One write, so it needs no transaction to be atomic — and outside one the
  // send above does not hold the write connection open across the network.
