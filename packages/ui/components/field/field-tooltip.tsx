...
import { useFieldPageCoords } from '@hanzo/esign-lib/client-only/hooks/use-field-page-coords';
...
const tooltipVariants = cva('font-semibold', {
  variants: {
    color: {
      ...
      warning: 'border-0 bg-foreground fill-foreground text-background',
    },
  },
  defaultVariants: {
    color: 'default',
...
interface FieldToolTipProps extends VariantProps<typeof tooltipVariants> {
  ...
  className?: string;
  // Only whether the field carries meta decides the tooltip, so the meta may be unparsed.
  field: Pick<
    Field,
